...
- [ ] Explain the differences between `client-side routing` and `server-side routing`.
      Server side routing is when a user requests info from a server through a website and the website grabs that info from a server.  this requires a page refresh however.

      Client side routing is the requesting of data internally.  It doesn't cause the page to refresh and is a quicker and cleaner user experience.  Data is grabbed from the state instead of a server
...
- [ ] Mention three tools we can use to make AJAX requests.
    ...
    there is axios, superagent, & fetch to name three
